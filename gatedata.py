...
def short_path(start, end, matrix, end_orien):
    curr = start
    coordinates = [start]
    ...
    if end_orien == "left":
        ...
        while (not equal1(curr, left)) and (not equal1(curr, right)):
            if abs(curr[1] - end[1]) == 1:
                # moves up or down
                if curr[0] < end[0]:
                    curr = [curr[0] + 1, curr[1]]
                else:
    ...
    for p in phdl["parts"]:
        for k in range(0, len(p["external"])):
            if p["external"][k]["inout"] == "in":
                if p["external"][k]["name"] in end_dic:
                    if ((p["name"] == "Mux") or (p["name"] == "Dmux")) and (p["internal"][k]["name"] == "sel"):
                        end_dic[p["external"][k]["name"]].append(["top", p["coord"]])
                    else:
                        end_dic[p["external"][k]["name"]].append(["left", p["coord"]])
                else:
                    if ((p["name"] == "Mux") or (p["name"] == "Dmux")) and (p["internal"][k]["name"] == "sel"):
                        end_dic[p["external"][k]["name"]] = [["top", p["coord"]]]
                    else:
                        end_dic[p["external"][k]["name"]] = [["left", p["coord"]]]
        ...
        for k in range(0, len(p["external"])):
            if (p["external"][k]["inout"] == "out") and (p["external"][k]["overall"] == "none"):
                for end in end_dic[p["external"][k]["name"]]:
                    ...
                    if end[0] == "top":
                        new_end = sum1(end[1], [-1, 0])
                    else:
                        new_end = sum1(end[1], [0, -1])
                    path = short_path(new_start, new_end, gmatrix, end[0])
                    for b in path:
                        wm[b[0]][b[1]] += 1
                    path.append(end[1])
